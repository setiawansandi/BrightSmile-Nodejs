...
exports.getSchedule = async (req, res, next) => {
  try {
    const result = await appointmentService.getSchedule(req.query.doctor, req.query.date);
...
exports.updateAppointment = async (req, res, next) => {
  try {
    const result = await appointmentService.updateAppointment(req.user.id, req.body);
    res.status(200).json({ code: 200, data: result });
  } catch (err) {
    if (err.message === 'invalid input')
      return res.status(400).json({ code: 400, error: 'invalid input' });
